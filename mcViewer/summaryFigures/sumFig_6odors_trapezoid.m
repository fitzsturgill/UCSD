function h = sumFig_6odors_trapezoid(trode, clusterInd, cycles)
    global state
...
%   params.matpos defines position of axesmatrix [LEFT TOP WIDTH HEIGHT].
    ...
    matpos_raster = [0 .3 .3 .7];    
    matpos_cluster = [.3 .3 .7 .7];
...
%% 2) Trial Histograms, rasters, etc:   By Cycle
    ...
    setaxesOnaxesmatrix(ax2(1), 1, 1, 1, params, h);    
    
    
    
    % axes matrix params for bar graph and histogram
...
%     end
    ...
    setaxesOnaxesmatrix(ax2(2:3), 2, 2, [1 3], params, h);
     scaleWidth(ax2(2), 2);
     scaleWidth(ax2(3), 2);
