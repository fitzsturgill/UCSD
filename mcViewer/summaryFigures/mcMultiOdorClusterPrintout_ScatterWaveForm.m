...
function h = mcMultiOdorClusterPrintout_ScatterWaveForm(trode, clusterInd, cycles, reject)
    global state
...
%     fun=state.mcViewer.trode(trode).cluster(clusterInd).analysis.WaveForm.displayFcnHandle;
    mcPlotWaveform(trode, clusterInd, h);
    axHandles(1) = gca;
...
%% 2) Trial Histograms, rasters, etc:   By Cycle
    ...
    for i = 0:size(cycles, 1) - 1 % number of rows in cycles....
        % raster 
        ax2(1 + i*2)=mcMakeRaster(trode, cluster, cycles(i+1, :), h, reject);  %odd positions in axes matrix are rasters
        set(gca, 'FontSize', 7);
        xlabel('time (sec)');
        ...
        % Trial Histogram    
        ax2(2 + i*2) = mcTrialHistogram(trode, clusterInd,cycles(i+1, :), h, state.mcViewer.ssb_alignHist, reject); % even positions in axes matrix are trial histograms
        set(gca, 'FontSize', 7);      
        xlabel('Time (sec)');
        ylabel('Spikes/s');
...
%     scaleWidth(ax2(3), 2);
%     scaleWidth(ax2(5), 3);
%     filepath  = ['C:\Fitz\Data\Analysis\Analysis_PirC_SOM\Arch_AAseries_searchForPVCells\combined\fast_clusters\' pageName];
