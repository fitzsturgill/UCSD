function h = mcMultiOdorClusterPrintout_Scatter(trode, clusterInd, cycles)
    global state
...
%     set(title_ax, 'HandleVisibility', 'off');
...
%% 2) Trial Histograms, rasters, etc:   By Cycle
    ...
    for i = 0:size(cycles, 1) - 1 % number of rows in cycles....
        ...
        xlabel('Time (sec)');
        ylabel('Spikes/s');
        addStimulusBar(ax2(2+i*2), [bl1 bl2 1], '', state.mcViewer.lineColor{1, 1}, 1);
        addStimulusBar(ax2(2+i*2), [x1 x2 1], '', state.mcViewer.lineColor{1, 2}, 1);
    end
...
%     end
        ...
        %% scatter plot
        ax2(end + 1) = mcScatterPlot2(trode, clusterInd, [1 2; 3 4; 1 2; 5 6; 1 2; 7 8], h); % hard coded...
        
       
    
    setaxesOnaxesmatrix(ax2, rows, columns, [1:length(ax2)], params, h);
    scaleWidth(ax2(end), 2);
%     scaleWidth(ax2(5), 3);
...
function ax=mcScatterPlot2(trode, clust, cycles, fig)
    global state
    ...
    if rem(size(cycles, 1), 2) ~= 0
        disp('error in mcScatterplot: cycles must be a 2*n row matrix, 1st row in a pair- xaxis, 2nd row in a pair- yaxis');
        return
    end
    ...
    counter = 1;
    colorSpec = {'r', '', 'g', '', 'b'};
    while counter+1 <= size(cycles, 1) 
        
        xData = [state.mcViewer.trode(trode).cluster(clust).analysis.SpikeRate.data(cycles(counter, :), 1).avg];
        yData = [state.mcViewer.trode(trode).cluster(clust).analysis.SpikeRate.data(cycles(counter+1, :), 1).avg];    
        scatter(xData, yData, colorSpec{counter});
    
            % linear fit
        cfun = fit(xData', yData', 'poly1');
        plot(cfun, colorSpec{counter});
        legend('off');
