function h = mcMultiOdorClusterPrintoutMUA_delayLED_reviewExp(trode, clusterInd, cycles, reject)
    % for review experiments, add trial spectrogram to 3 amyl acetate
...
%         ax2(end+1) = mcScatterPlot(trode, clusterInd, cycles', h);
        ax2 = [ax2 mcAveragePowerSpectrum('h', h)];
        %% quality
%     % plot stability
%     axes('Parent', h);
%     ax2(end+1) = gca;
