...
function odorMod = plotMultiOdorCluster_RankOrder_raw(indices, SpikesByRespCycle)
    % returns values of maximal fractional odor increase in FR relative to
    % air trial, value less than 1 if all odors are inhibitory 
    global clusters
...
%             deltaRate = odor - repmat(mean(baseline, 2), 1, 11);
            
            
          
            [B IX] = sort(odor(1, 2:end));
            ...
            odor = odor(:, IX);
            odorMod = [odorMod odor(:, end - 1) ./ odor(:, end)];
            odor_std = odor_std(:, IX);
            odor_N = odor_N(:, IX);
            odor_sem = odor_std ./ sqrt(odor_N);
%             errorbar([1:11;1:11]', deltaRate', odor_sem');            
            errorbar(1:nOdors, odor(1, :), odor_sem(1, :), 'k');
            errorbar(1:nOdors, odor(2, :), odor_sem(2, :), 'r');
            title(gca,...
            ...
            ylabel('RawFiring Rate', 'FontSize', 7);
        end
        splayAxisTile;
