function [varargout] = AAseries_linearFits_deltaFR(indices, Rthresh)
...
%     Rthresh- threshold R value for units to include in grandAverage
    %% make a mosaic of all the individual fits
        ...
        for j = 1:length(indices)
            ...
            count = count + 1;
            xData = [clusters(ind).analysis.SpikeRate.data([1 3:numconc], 1).avg] - mean([clusters(ind).analysis.SpikeRate.data([1 3:numconc], 2).avg]);
            yData = [clusters(ind).analysis.SpikeRate.data([1 3:numconc] + numconc, 1).avg] - mean([clusters(ind).analysis.SpikeRate.data([1 3:numconc] + numconc, 2).avg]);
