function varargout = AAseries_linearFits(indices, Rthresh)
...
%     Rthresh- threshold R value for units to include in grandAverage
    %% make a mosaic of all the individual fits
        ...
        for j = 1:length(indices)
            ...
            count = count + 1;
            xData = [clusters(ind).analysis.SpikeRate.data([1 3:numconc], 1).avg];
            yData = [clusters(ind).analysis.SpikeRate.data([1 3:numconc] + numconc, 1).avg];
    ...
    summaryFig = figure;
        ...
        for j = 1:length(indices)
            ...
            if abs(Rvalues(1, j)) < Rthresh
                disp(['Excluded index ' num2str(indices(j))]);
                continue
            end
            ind = indices(j);
            numconc = size(clusters(ind).analysis.SpikeRate.data, 1) / 2;
