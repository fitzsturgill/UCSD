...
function mcSaveSpikes(varargin)
    ...
    figdata.spikes.params.display.default_waveformmode=2; % default is "Bands" for use within mcViewer
    state.mcViewer.trode(index).spikes = figdata.spikes;
    try
        mcUpdateSpikeLines(1); %ensures that cluster structure is created
    catch
        disp('kludge in splitmerge_tool, mcSaveSpikes: skipped mcUpdateSpikeLines(1), tsData may not be loaded due to size of data set (very large)');
    end
end
% --------------------------------
