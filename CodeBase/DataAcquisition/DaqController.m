...
function DaqController_OpeningFcn(hObject, eventdata, handles, varargin)
    % FS MOD 12/15/11
    global state gh
    gh.DaqControllerAux = guihandles(DaqControllerAux);
    openini('DaqController.ini');
    evalin('base', 'global AIOBJ');
...
% FS MOD- just put RigDef somewhere global for easy reference
state.DaqControllerAux.RigDef=RigDef;
...
% Determine whether board allows TriggerType, 'HwDigital'; if not, use
% 'Software' trigger
aiTemp = analoginput('nidaq',RigDef.equipment.board.device);
temp = propinfo(aiTemp,'TriggerType');
temp = strcmp(temp.ConstraintValue,'HwDigital');
if any(temp)
    handles.board.TriggerType = 'HwDigital';
else
...
global UDP_OBJ_STIM_PC savePrefix %FS MOD
...
handles.aiparams.savefile_header = [savePrefix '_1'];  %FS MOD
...
handles.aiparams.TriggerType = 'Immediate'; %FS MOD
...
if get(handles.checkbox2_AutoRestart,'Value') == 1;
    handles.aiparams.StopFcn = {@start}; %FS MOD- let timer execute
%     start- see below in changeai....
%       handles.aiparams.StopFcn = {@dummyStop};
else
...
%FS MOD-       
% if strcmp(AIOBJ.TriggerType,'Software')
    AIOBJ.TimerFcn = handles.aiparams.TimerFcn; 
% elseif strcmp(AIOBJ.TriggerType, 'Immediate')
%     AIOBJ.TimerFcn = {@start};
% end
% %
...
if status == 1 % Start data acquisition and disable other GUI elements
    %FS MOD
    state.DaqControllerAux.currentCycle=1;
    updateGUIByGlobal('state.DaqControllerAux.currentCycle');
    state.DaqControllerAux.currentAcq=0;
    updateGUIByGlobal('state.DaqControllerAux.curentAcq');
    %
