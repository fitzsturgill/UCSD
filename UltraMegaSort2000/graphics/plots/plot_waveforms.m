...
%     data.ylims = [min(spikes.waveforms(:)) max(spikes.waveforms(:))];
    yminAvg = mean(min(min(spikes.waveforms, [], 2),[], 3));
    yminStd = std(min(min(spikes.waveforms, [], 2), [], 3));
    ymaxAvg = mean(max(max(spikes.waveforms, [], 2), [], 3));    
    ymaxStd = std(max(max(spikes.waveforms, [], 2), [], 3));        
    data.ylims = [(yminAvg - 5*yminStd) (ymaxAvg + 5*ymaxStd)];
